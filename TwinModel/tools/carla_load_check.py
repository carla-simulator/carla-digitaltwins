...
sys.path.insert(0, "/home/german/Projects/CARLA_SOURCE/carla-digitaltwins/TwinModel")
...
def main() -> int:
    ...
    params = carla.OpendriveGenerationParameters(
        vertex_distance=2.0, max_road_length=50.0, wall_height=0.0, additional_width=0.6,
        smooth_junctions=True, enable_mesh_visibility=True, enable_pedestrian_navigation=False)
    t0 = time.perf_counter()
    world = client.generate_opendrive_world(xodr, params)
    result["generate_seconds"] = round(time.perf_counter() - t0, 1)
    log(f"generate_opendrive_world done in {result['generate_seconds']} s, map {world.get_map().name}")
