...
def build_lanegraph(osm: OsmData, frame: LocalFrame, bbox: tuple[float, float, float, float],
    ...
    for ch in chains:
        ...
        if a in intersection and b in intersection and a != b:
            ...
            spans_levels = any(is_bridge(s.way.tags) or osm_layer(s.way.tags) != 0
                               for s in ch.segments)
            radius = (P.junction.gore_cluster_m if (grade_separated or spans_levels)
                      else P.junction.cluster_m)
            if (_polyline_length(ch.xy) < radius
                    and math.dist(node_xy[a], node_xy[b]) < radius):
                uf.union(a, b)
    ...
    def roads_from_chains(node_cluster: dict[int, _Cluster]):
        ...
        for ch in chains:
            ...
            # a freeway has no street space: buildings flanking it are set back behind the
            # right of way, never a canyon that would give it sidewalks or parking lanes
            faces = (None if road.highway in P.lane.grade_separated_classes
        ...
        for inc in (() if c.kind == "gore" else incoming):
            ...
            for out, turn, forced in legal:
                ...
                for in_lane, out_lane in _lane_pairs(inc.lanes, out.lanes, mapping_turn,
                    ...
                    movements.append((inc, in_lane, out, out_lane, turn))
        if c.kind == "gore":
            # a gore has no turns: the lane count simply changes along the mainline
            movements = _gore_movements(incoming, outgoing)
        for inc, in_lane, out, out_lane, turn in movements:
            m += 1
