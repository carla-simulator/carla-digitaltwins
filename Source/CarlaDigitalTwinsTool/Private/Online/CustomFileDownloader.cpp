...
void UCustomFileDownloader::ConvertOSMInOpenDrive(FString FilePath, float Lat_0, float Lon_0)
{
  ...
  if (PlatformFile.FileExists(*FilePath))
  {
    ...
    {
      UE_LOG(LogCarlaDigitalTwinsTool, Display, TEXT("FileManipulation: Did not load text from file"));
    }
  }
  else
  {
    UE_LOG(LogCarlaDigitalTwinsTool, Display, TEXT("File: %s does not exist"), *FilePath);
    return;
  }
  ...
  if (Ar->IsError())
  {
      UE_LOG(LogCarlaDigitalTwinsTool, Error, TEXT("ConvertOSMInOpenDrive: Ar->IsError() == true. File:%s"), *FilePath);
  }
  if (Ar->IsCriticalError())
  {
      UE_LOG(LogCarlaDigitalTwinsTool, Error, TEXT("ConvertOSMInOpenDrive: Ar->IsCriticalError() == true. File:%s"), *FilePath);
  }
}

void UCustomFileDownloader::StartDownload()
{
  UE_LOG(LogCarlaDigitalTwinsTool, Display, TEXT("FHttpDownloader CREATED"));
  UE_LOG(LogCarlaDigitalTwinsTool, Display, TEXT("Map Name Is %s"), *ResultFileName );
...
{
  UE_LOG(LogCarlaDigitalTwinsTool, Display, TEXT("Starting download [%s] Url=[%s]"), *Verb, *Url);
  TSharedPtr<IHttpRequest, ESPMode::ThreadSafe> Request = FHttpModule::Get().CreateRequest();
  UE_LOG(LogCarlaDigitalTwinsTool, Display, TEXT("Map Name Is %s"), *Filename );
...
void FHttpDownloader::RequestComplete(FHttpRequestPtr HttpRequest, FHttpResponsePtr HttpResponse, bool bSucceeded)
{
  ...
  {
    UE_LOG(LogCarlaDigitalTwinsTool, Display, TEXT("Download failed. NULL response"));
  }
  else
  {
    ...
    UE_LOG(LogCarlaDigitalTwinsTool, Display, TEXT("Completed download [%s] Url=[%s] Response=[%d]"),
           *HttpRequest->GetVerb(),
           *HttpRequest->GetURL(),
           HttpResponse->GetResponseCode());

    FString CurrentFile = UGenerationPathsHelper::GetRawMapDirectoryPath(Filename) + "OpenDrive/";
    UE_LOG(LogCarlaDigitalTwinsTool, Display, TEXT("FHttpDownloader::RequestComplete CurrentFile %s."), *CurrentFile);

    // We will use this PlatformFile to deal with the file.
    IPlatformFile &PlatformFile = FPlatformFileManager::Get().GetPlatformFile();
    if (!PlatformFile.DirectoryExists(*CurrentFile))
    {
      PlatformFile.CreateDirectory(*CurrentFile);
    }
    ...
    {
      UE_LOG(LogCarlaDigitalTwinsTool, Display, TEXT("FileManipulation: Successfully Written "));
      DelegateToCall.ExecuteIfBound();
    }
    else
    {
      UE_LOG(LogCarlaDigitalTwinsTool, Display, TEXT("FileManipulation: Failed to write FString to file."));
      UE_LOG(LogCarlaDigitalTwinsTool, Display, TEXT("FileManipulation: CurrentFile %s."), *CurrentFile);
    }
  }
  delete this;
}
